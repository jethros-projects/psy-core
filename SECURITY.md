...
psy-core is a local receipt layer, not a vault. It makes memory tampering visible under a clear local threat model: the audit rows are hash-chained, the tail is HMAC-sealed, and `psy verify --all` walks the active DB, archives, intent/result pairs, and sealed head.

The promise is intentionally narrow: naive row edits break hash verification; rewritten chains, truncation, and substitution break the sealed-head check when the seal key is still trusted. Do not use psy-core docs to imply tamper-proof storage or regulatory compliance.
...
- **Result-row failure after the handler already ran.** The in-process Node wrappers record `intent` before the handler executes, so an intent-write failure prevents the provider call. If the audit DB write fails AFTER the memory store mutation succeeded, the result row is missing while the mutation persisted. `psy verify` flags the orphaned `intent` so this state is detectable. External observer plugins such as Hermes/OpenClaw are best-effort observers at their host hook boundary, not provider transactions. Closing this gap fully (audit DB + memory store sharing one transaction) is outside psy-core's current scope.
...
- **Bootstrap key consistency.** File-based key bootstrap uses atomic create-if-absent publishing; concurrent initializers converge on the key that reaches disk first and losers reload it. Do not run different processes with different `PSY_SEAL_KEY` environment values against the same DB: they will produce head pointers that cannot verify under one another's keys.
...
- **Initialization marker.** Running `psy init` or the public SDK `init()` on a fresh project marks `.psy.json` with `seal: "required"`, which makes `psy verify` fail with `seal_missing_required` if `head.json` is later wiped. Projects that bootstrap implicitly by calling `wrap()` without any init step leave the marker as `optional` - meaning a wipe of `.psy/head.json` AND `.psy/seal-key` together would silently downgrade verification on those installs. SDK-only users who want the downgrade-attack defense should call `init()` once, run `psy init`, or set `seal: "required"` in `.psy.json` manually.
