"""psy-core-hermes — tamper-evident memory audit for Hermes Agent.
...
Plugin entry point: `psy_core.hermes.register` (see `register.py`).
"""
...
# at the package root: doing so would rebind `psy_core.hermes.register` (the
# submodule) to the function, breaking `import psy_core.hermes.register` and
